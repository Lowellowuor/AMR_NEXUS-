  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Sector */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Sector *</label>
        ...
        {/* Sub-sector */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Subâ€‘sector *</label>
        ...
        {/* Pathogen Code */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Pathogen Code *</label>
        ...
        {/* Specimen Type */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Specimen Type *</label>
          <input {...register('specimen_type')} className="input mt-1 w-full" placeholder="e.g., Blood, Urine, Swab" />
        </div>

        {/* County */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">County *</label>
          <input {...register('county')} className="input mt-1 w-full" placeholder="e.g., Nairobi" />
        </div>

        {/* Antibiotic Class */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Antibiotic Class *</label>
        ...
        {/* Test Method */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Test Method *</label>
          <input {...register('test_method')} className="input mt-1 w-full" placeholder="e.g., Disk diffusion" />
        </div>

        {/* Sample Month */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Sample Month *</label>
          <input type="number" {...register('sample_month', { valueAsNumber: true })} className="input mt-1 w-full" min="1" max="12" />
        </div>

        {/* Prior Antibiotic Exposure */}
        <div className="flex items-center">
          <input type="checkbox" {...register('prior_antibiotic_exposure')} className="h-4 w-4 text-[var(--accent-teal)] focus:ring-primary-500 border-[var(--border-secondary)] rounded" />
          <label className="ml-2 block text-sm text-[var(--text-secondary)]">Prior antibiotic exposure (last 30 days)</label>
        </div>

        {/* Hospitalised */}
        <div className="flex items-center">
          <input type="checkbox" {...register('hospitalised')} className="h-4 w-4 text-[var(--accent-teal)] focus:ring-primary-500 border-[var(--border-secondary)] rounded" />
          <label className="ml-2 block text-sm text-[var(--text-secondary)]">Hospitalised</label>
        </div>

        {/* Gender */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Gender</label>
        ...
        {/* Age Group */}
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Age Group</label>
          <input {...register('age_group')} className="input mt-1 w-full" placeholder="e.g., Adult, Child, 60+ years" />
        </div>

        {/* Facility */}
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-[var(--text-secondary)]">Facility (optional)</label>
      ...
      <div className="flex justify-start pt-4">
        <button
          type="submit"
          disabled={isLoading}
          className="px-8 py-2.5 bg-[var(--accent-teal)] hover:bg-[var(--accent-teal)] text-white font-medium rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {isLoading ? (
            <>
