...
export default function AlertCard({
  ...
  const severityColors = {
    high: 'border-l-[var(--status-critical)] bg-[var(--status-critical-bg)]/50',
    medium: 'border-l-[var(--status-warning)] bg-[var(--status-warning-bg)]/50',
    low: 'border-l-blue-500 bg-blue-50/50',
  };
  const severityBadge = {
    high: 'bg-[var(--status-critical-bg)] text-red-800',
  ...
  return (
    <div
      className={`bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border-l-4 overflow-hidden transition-all ${
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="text-xs text-[var(--text-muted)]">{new Date(alert.timestamp || alert.created_at).toLocaleString()}</span>
              {isRead && (
            ...
            {alert.details && <p className="text-sm text-[var(--text-muted)] mt-1">{alert.details}</p>}
            {alert.shap_summary && (
              <p className="text-xs text-[var(--text-secondary)] mt-2 bg-[var(--bg-tertiary)] p-2 rounded-lg border border-[var(--border-primary)]">
                <span className="font-semibold">SHAP:</span> {alert.shap_summary}
              </p>
            )}
            <button
              onClick={handleToggleGuidance}
              className="mt-2 text-xs text-[var(--accent-teal)] hover:text-[var(--accent-teal)] font-medium"
            >
              {isExpanded ? 'Hide guidance' : 'Show guidance'}
            </button>
            {isExpanded && displayGuidance && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-[var(--text-secondary)] whitespace-pre-wrap">
                {displayGuidance}
              </div>
            )}
            {isExpanded && loadingGuidance && (
              <div className="mt-3 p-3 bg-[var(--bg-tertiary)] rounded-lg text-sm text-[var(--text-muted)]">Loading guidance...</div>
            )}
          </div>
          <div className="flex gap-2 ml-4">
            {!isRead && (
              <button
                ...
                className="text-xs bg-primary-100 text-[var(--accent-teal)] px-2 py-1 rounded-full hover:bg-primary-200"
              >
                Acknowledge
              </button>
            )}
