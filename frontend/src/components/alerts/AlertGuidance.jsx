  ...
  return (
    <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
      <div className="flex items-start gap-3">
        <LightBulbIcon className="h-5 w-5 text-[var(--accent-blue)] mt-0.5 flex-shrink-0" />
        <div className="flex-1">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-semibold text-blue-800">
              ðŸ“‹ Stewardship Guidance
            </h4>
            {!loading && !error && (
              <button
                onClick={fetchGuidance}
                className="text-xs text-[var(--accent-blue)] hover:text-blue-800 flex items-center gap-1"
              >
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
              <div className="animate-spin h-4 w-4 border-2 border-[var(--accent-blue)] border-t-transparent rounded-full"></div>
              Loading guidance...
            </div>
          )}

          {error && (
            <div className="text-sm text-[var(--status-critical)]">
              <p>{error}</p>
              <button
                onClick={handleRetry}
                className="mt-1 text-xs text-[var(--accent-blue)] underline hover:text-blue-800"
              >
                Retry
              </button>
            </div>
          ...
          {guidance && !loading && !error && (
            <>
              ...
              {guidance.recommendations && guidance.recommendations.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-medium text-[var(--text-secondary)]">Recommendations:</p>
