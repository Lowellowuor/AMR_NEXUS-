  ...
  return (
    <div className="fixed bottom-5 right-5 z-[200] space-y-3 max-w-sm">
      {visibleAlerts.map(alert => {
        ...
        const borderColor = isCritical ? 'border-l-[var(--status-critical)]' : isHigh ? 'border-l-red-400' : 'border-l-teal-500';
        const bgColor = isCritical
          ? 'bg-[var(--status-critical-bg)]/95 dark:bg-red-950/90'
          : isHigh
          ? 'bg-[var(--status-critical-bg)]/80 dark:bg-red-950/70'
          : 'bg-teal-50/95 dark:bg-teal-950/70';

        const iconColor = isCritical || isHigh ? 'text-[var(--accent-red)]' : 'text-[var(--accent-cyan)]';
        const iconBg = isCritical || isHigh ? 'bg-[var(--status-critical-bg)] dark:bg-red-900/30' : 'bg-teal-100 dark:bg-teal-900/30';
