...
import { fetchAlertDetail, fetchAlertExplanation, generateLLMResponse } from '../../api/endpoints';
...
export default function AlertDetailModal({ alertId, role, open, onClose }) {
  ...
  const { data: llmResponse, isLoading: llmLoading } = useQuery({
    queryKey: ['llm-guidance', alertId],
    queryFn: () => generateLLMResponse(alertId),
    enabled: !!alertId && open,
  });

  if (!open) return null;

  const isLoading = alertLoading || explLoading || llmLoading;
          ...
          <>
            ...
            <div className="p-6 border-b border-[var(--border-primary)]">
              ...
              <div className="flex items-start gap-4 mb-4">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${isCritical ? 'bg-[var(--status-critical)]/10' : isHigh ? 'bg-[var(--status-critical)]/10' : 'bg-cyan-500/10'}`}>
                  <AlertTriangle className={`w-5 h-5 ${isCritical || isHigh ? 'text-[var(--accent-red)]' : 'text-[var(--accent-cyan)]'}`} />
                </div>
            ...
            {explanation && (
              <div className="p-6 border-b border-[var(--border-primary)]">
                ...
                <div className={`p-4 rounded-xl border mb-4 ${isCritical || isHigh ? 'bg-[var(--status-critical)]/5 border-[var(--status-critical)]/20' : 'bg-cyan-500/5 border-cyan-500/20'}`}>
                  <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{explanation.plainTextSummary}</p>
                  <p className="text-xs text-[var(--text-muted)] mt-2">
                    Confidence: {(explanation.confidence * 100).toFixed(0)}% · Model: {explanation.model_version}
                  </p>
                </div>
                      ...
                      {explanation.contributors.map((c, idx) => (
                        <div key={idx}>
                          <div className="flex justify-between text-sm mb-1.5">
                            <span className="text-[var(--text-secondary)]">{c.factor} <span className={`text-xs ${c.direction === 'increases risk' ? 'text-red-400' : 'text-emerald-400'}`}>({c.direction})</span></span>
                            <span className="font-semibold text-[var(--text-primary)]">{Math.abs(c.shap_value).toFixed(3)}</span>
                          </div>
                          <div className="h-2 bg-[var(--bg-tertiary)] rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${c.direction === 'increases risk' ? 'bg-[var(--status-critical)]' : 'bg-[var(--status-success-bg)]0'}`}
                              style={{ width: `${Math.min(Math.abs(c.shap_value) * 100, 100)}%` }}
                            />
                          </div>
                        </div>
                      ))}
            ...
            {/* Step 3: Recommended Actions (Claude) */}
            {llmResponse && (
              <div className="p-6 border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2 mb-4">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--accent-teal)]/20 text-teal-400 text-xs font-bold">3</span>
                  <h3 className="font-semibold text-[var(--text-primary)]">Recommended Actions</h3>
                </div>
                <div className="p-4 rounded-xl bg-teal-50/50 dark:bg-teal-950/20 border border-teal-200/50 dark:border-teal-800/50">
                  <p className="text-sm text-[var(--text-secondary)] whitespace-pre-line leading-relaxed">{llmResponse.text}</p>
                </div>
              </div>
            )}
            ...
            <div className="p-6">
              <div className="flex items-center gap-2 mb-4">
                <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--status-warning-bg)]0/20 text-amber-400 text-xs font-bold">4</span>
                <h3 className="font-semibold text-[var(--text-primary)]">Notification</h3>
              </div>
              <p className="text-sm text-[var(--text-secondary)] mb-4">Dispatch SMS alert to {alert.county} County AMR Focal Person.</p>
              <button
                onClick={() => {
