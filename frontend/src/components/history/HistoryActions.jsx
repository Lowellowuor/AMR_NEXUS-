...
export default function HistoryActions({ recordId, onDelete, onShare }) {
  const handleDelete = async () => {
    if (window.confirm('Permanently delete this prediction? This action cannot be undone.')) {
      try {
        const response = await fetch(`http://localhost:8000/predictions/${recordId}`, { method: 'DELETE' });
  ...
  return (
    <div className="flex gap-2">
      <button onClick={handleShare} className="p-1 text-[var(--text-muted)] hover:text-[var(--accent-blue)] transition" title="Share">
        <ShareIcon className="h-4 w-4" />
      </button>
      <button onClick={handleDelete} className="p-1 text-[var(--text-muted)] hover:text-[var(--status-critical)] transition" title="Delete">
