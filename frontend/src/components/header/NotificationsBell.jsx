import { BellIcon, ExclamationTriangleIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
      ...
      {count > 0 && (
        <div className="hidden group-hover:block absolute top-full right-0 mt-1 w-80 bg-[var(--bg-secondary)] rounded-xl shadow-lg border p-2 z-50">
          ...
          {alerts.map(a => (
            <Link key={a.id} to="/alerts" className="block px-2 py-1 text-sm hover:bg-[var(--bg-tertiary)] rounded">
              <span className="inline-flex items-center gap-1">
                <ExclamationTriangleIcon className="h-3.5 w-3.5 text-[var(--status-critical)] inline" />
                <span>{a.message}</span>
              </span>
          ...
          <div className="border-t mt-1 pt-1 text-center">
            <Link to="/alerts" className="text-xs text-[var(--accent-teal)] inline-flex items-center gap-1">
              View all
