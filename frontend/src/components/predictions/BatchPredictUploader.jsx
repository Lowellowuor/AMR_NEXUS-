...
export default function BatchPredictUploader({ onBatchComplete }) {
  ...
  return (
    <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-6">
      <h3 className="text-lg font-semibold text-[var(--text-primary)] flex items-center gap-2 mb-4">
        <DocumentArrowUpIcon className="h-5 w-5 text-[var(--accent-teal)]" />
        Batch Upload & Predict
      </h3>
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-[var(--border-primary)] hover:bg-gray-300 text-[var(--text-primary)] rounded-full transition font-medium">
          ...
          {file && (
            <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
              ...
              <button onClick={clearFile} className="text-[var(--text-muted)] hover:text-[var(--status-critical)] transition">
        ...
        {rows.length > 0 && (
          <div className="overflow-x-auto max-h-48 overflow-y-auto">
            <p className="text-sm text-[var(--text-muted)] mb-2 sticky top-0 bg-[var(--bg-secondary)]/80 backdrop-blur-sm py-1">
              Preview ({rows.length} rows)
              <span className="ml-2 text-xs text-[var(--text-muted)]">(first 5 shown)</span>
            </p>
            <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
            <table className="min-w-full text-xs border-collapse">
              <thead className="bg-[var(--bg-tertiary)] sticky top-6">
                <tr>
                  {headers.map((key, idx) => (
                    <th key={idx} className="border px-2 py-1 text-left font-medium text-[var(--text-secondary)]">
                      {key}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {rows.slice(0, 5).map((row, idx) => (
                  <tr key={idx} className="hover:bg-[var(--bg-tertiary)]">
                    {headers.map((key, i) => (
                      <td key={i} className="border px-2 py-1 text-[var(--text-secondary)] truncate w-full max-w-[100px]">
                        {String(row[key] ?? '')}
                      </td>
                    ))}
                  </tr>
                ...
                {rows.length > 5 && (
                  <tr>
                    <td colSpan={headers.length} className="text-center text-[var(--text-muted)] py-1">
        ...
        {progress.total > 0 && (
          <div className="space-y-1">
            ...
            <div className="w-full bg-[var(--border-primary)] rounded-full h-2.5 overflow-hidden">
              <div
                className="bg-[var(--accent-teal)] h-2.5 rounded-full transition-all duration-300 ease-out"
        ...
        {results.length > 0 && (
          <div className="mt-3 p-3 bg-[var(--bg-tertiary)] rounded-xl max-h-48 overflow-y-auto">
            <div className="flex items-center gap-4 text-sm mb-2">
              ...
              {failCount > 0 && (
                <span className="text-[var(--status-critical)] flex items-center gap-1">
            ...
            <div className="space-y-1">
              {results.map((r, idx) => (
                <div
                  key={idx}
                  className={`text-sm flex items-center gap-2 ${r.success ? 'text-[var(--status-success)]' : 'text-[var(--status-critical)]'}`}
                >
        ...
        <div className="pt-2 border-t border-[var(--border-primary)] flex flex-wrap items-center gap-3">
          <button
