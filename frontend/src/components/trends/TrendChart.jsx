...
export default function TrendChart({ pathogen, drug, region, months = 12 }) {
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${trendUp ? 'bg-[var(--status-critical)]/10' : 'bg-cyan-500/10'}`}>
            {trendUp ? <TrendingUp className="w-5 h-5 text-[var(--accent-red)]" /> : <TrendingDown className="w-5 h-5 text-[var(--accent-cyan)]" />}
          </div>
          <div>
            <h3 className="font-semibold text-base text-[var(--text-primary)]">{pathogen}</h3>
            <p className="text-sm text-[var(--text-secondary)]">{drug} Resistance · {region} · {months} months</p>
          </div>
        </div>
      ...
      {hasAnomaly && (
        <div className="mt-4 p-4 rounded-xl bg-[var(--status-critical)]/10 border border-[var(--status-critical)]/20 flex gap-3">
          <AlertTriangle className="w-5 h-5 text-[var(--accent-red)] flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-semibold text-[var(--accent-red)]">Anomaly Detected — Significant Resistance Spike</p>
