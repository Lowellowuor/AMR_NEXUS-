...
export default function AnomalyTimeline({ startDate, endDate, county }) {
  ...
  if (loading) {
    return (
      <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-5 flex items-center justify-center h-64">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--accent-teal)] mx-auto"></div>
          <p className="mt-2 text-[var(--text-secondary)] text-sm">Loading anomalies...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-5 flex items-center justify-center h-64">
        <div className="text-center text-[var(--status-critical)]">
          <p>{error}</p>
          <button onClick={() => window.location.reload()} className="mt-2 text-[var(--accent-teal)] underline text-sm">
            Retry
          </button>
        </div>
      </div>
  ...
  if (data.length === 0) {
    return (
      <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-5 flex items-center justify-center h-64">
        <p className="text-[var(--text-muted)] text-center">No anomalies detected in this period.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-5">
      <h3 className="text-md font-semibold mb-2 text-[var(--text-primary)]">Anomaly Timeline</h3>
