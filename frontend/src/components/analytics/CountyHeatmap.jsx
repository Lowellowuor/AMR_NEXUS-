...
function Legend({ metric, maxValue, colorScale }) {
  ...
  return (
    <div className="absolute bottom-6 left-6 z-[1000] bg-[var(--bg-secondary)]/90 backdrop-blur-sm p-3 rounded-xl shadow-md text-sm border border-white/50">
...
export default function CountyHeatmap({
  ...
  if (error) return <div className="text-center p-4 text-[var(--status-critical)]">{error}</div>;
  if (!geoJson) return <div className="text-center p-4">No map data available.</div>;

  return (
    <div className="relative">
      <div className="absolute top-4 left-4 z-[1000] flex gap-3 bg-[var(--bg-secondary)]/90 p-2 rounded-xl shadow-md">
        <select
          value={selectedMetric}
          onChange={handleMetricChange}
          className="border border-[var(--border-secondary)] rounded-lg px-3 py-1 text-sm focus:ring-2 focus:ring-primary-500"
        >
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-[var(--text-secondary)]">Months:</span>
          <input
            type="range"
            min="1"
            max="12"
