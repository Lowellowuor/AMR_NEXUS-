  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:8000/analytics/resistance_by_pathogen/${selected}?${qs}`);
        const data = await res.json();
        setResistanceData(data);
        // Trend (using pathogen-specific endpoint)
        const trendRes = await fetch(`http://localhost:8000/analytics/pathogen_trend?pathogen_code=${selected}&months=12&${qs}`);
        if (trendRes.ok) {
