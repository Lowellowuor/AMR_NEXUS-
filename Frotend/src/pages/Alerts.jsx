...
export default function Alerts() {
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      // Anomaly alerts
      predictions.filter(p => p.anomaly_detected).forEach(anomaly => {
        newAlerts.push({
          id: `anomaly-${anomaly.record_id}`,
          message: `?? Anomaly detected: Unusual resistance pattern for ${anomaly.pathogen_code?.toUpperCase()} in ${anomaly.county}`,
          timestamp: anomaly.timestamp,
          severity: 'medium',
          type: 'anomaly',
          acknowledged: false,
          details: 'This isolate showed an unusual resistance pattern not typical for this pathogen/location.'
        });
      });

      // High MDR trend alert
      if (summary.mdr_rate > 30) {
        newAlerts.push({
          ...
          type: 'trend',
          acknowledged: false,
          details: 'Consider reviewing antibiotic stewardship programmes in affected regions.'
        });
      }
